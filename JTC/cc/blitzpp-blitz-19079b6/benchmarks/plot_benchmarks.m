...
str=sprintf('%s\nCXX=c++ CXXFLAGS= -O3 -funroll-loops -fstrict-aliasing -fomit-frame-pointer -ffast-math -DUSE_LIBBLAS', str);
str=sprintf('%s\nF77=gfortran FFLAGS=-fno-second-underscore -O3 -funroll-loops', str);
if length('gfortran')
	str=sprintf('%s\nFC=gfortran FCFLAGS=-fno-second-underscore -ffree-form -O3 -funroll-loops', str);
end
str=strrep(str,'_','\_');
h=text(0.5,0.5,str);
